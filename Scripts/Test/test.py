...
def networktest(NodeList=list,EdgeList=list,NodeData=dict,EdgeData=dict):
    ...
    # print(DegCen)
    UniqVal=[]
    for data in DegCen.keys():
        # print(DegCen[data])
        if DegCen[data] not in UniqVal:
            UniqVal.append(DegCen[data])

    for d in UniqVal:
        print(d)

    nx.draw_networkx_nodes(G,pos=NodeData['Pos'],node_size=50)
    nx.draw(G)
    plt.show()
...
#     return Nodes,Edges,DataDict,EdgeData
